...
class PlayTransition : Transition {
    ...
    override fun createAnimator(
        ...
        val startView = addViewToOverlay(
            sceneRoot, startImage!!.width, startImage.height, startBackground
        )
        val shrinkingBackground: Drawable = ColorDrawable(color)
        val shrinkingView = addViewToOverlay(
            sceneRoot, startImage.width, startImage.height, shrinkingBackground
        )
        ...
        endView.alpha = 0f
        val shrinkingAnimator = createCircularReveal(
            shrinkingView, startRadius, minRadius.toFloat()
        )
        ...
        shrinkFadeSet.playTogether(
            shrinkingAnimator, startAnimator, fadeInAnimator
        )
        ...
        val circlePath: Path = pathMotion.getPath(
            circleStartX, circleStartY, circleEndX, circleEndY
        )
        val circleAnimator: Animator = ObjectAnimator.ofFloat(
            circleView, View.TRANSLATION_X, View.TRANSLATION_Y, circlePath
        )
        val growingView = addViewToOverlay(
            sceneRoot, endView.width, endView.height, shrinkingBackground
        )
    ...
    companion object {
        ...
        private val TRANSITION_PROPERTIES = arrayOf(
            PROPERTY_BOUNDS, PROPERTY_POSITION
        )
